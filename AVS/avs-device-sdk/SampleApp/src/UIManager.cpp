...
//SILOGOOD
void UIManager::alexa_auth_done(){
    system("rm /data/alexa_auth_done");
    std::string filePath = "/data/alexa_auth_done";
    std::ofstream writeFile(filePath.data());
    if( writeFile.is_open()){
        writeFile << "done";
        writeFile.close();
    }
    system("chmod 777 /data/alexa_auth_done"); 
}
...
void UIManager::printState() {
    ...
    //led blue
    std::string led_blue_max("echo 255 > /sys/class/leds/led_blue/brightness");
    std::string led_blue_zero("echo 0 > /sys/class/leds/led_blue/brightness");

    //led Green
    std::string led_green_max("echo 255 > /sys/class/leds/led_green/brightness");
    std::string led_green_zero("echo 0 > /sys/class/leds/led_green/brightness");

    //led Red
    std::string led_red_max("echo 255 > /sys/class/leds/led_red/brightness");
    std::string led_red_zero("echo 0 > /sys/class/leds/led_red/brightness");

    //led White
    std::string led_white_max("echo 255 > /sys/class/leds/led_white/brightness");
    std::string led_white_zero("echo 0 > /sys/class/leds/led_white/brightness");
    ...
    if (m_connectionStatus == avsCommon::sdkInterfaces::ConnectionStatusObserverInterface::Status::DISCONNECTED) {
        ...
        switch (m_dialogState) {
            case DialogUXState::IDLE:
                ...
                //SILOGOOD
                system(led_blue_zero.c_str());
                system(led_green_zero.c_str());
                system(led_white_zero.c_str());

                ConsolePrinter::prettyPrint("Alexa is currently idle!");
                return;
            case DialogUXState::LISTENING:

                system(led_blue_max.c_str());
                system(led_green_zero.c_str());
                system(led_white_zero.c_str());
