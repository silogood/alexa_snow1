...
//SILOGOOD
void UserInputManager::thread_function(){
    char eventPathStart[] = "/dev/input/event1";
    std::vector<pollfd> poll_fds;
    for (auto &i : std::__fs::filesystem::directory_iterator("/dev/input")) {
        ...
        for (auto &pfd : poll_fds) {
            if (pfd.revents & POLLIN) {
                if(read(pfd.fd, &ev, sizeof(ev)) == sizeof(ev)) {
                    // INNOPIA_ALEXA 0x0246   // Olli 0x0073
...
SampleAppReturnCode UserInputManager::run() {
    ...
    //SILOGOOD wakeword & spech confirm sound enable
    m_interactionManager->setWakewordConfirmation(settings::WakeWordConfirmationSettingType::TONE);
    m_interactionManager->setSpeechConfirmation(settings::SpeechConfirmationSettingType::TONE);

    //SILOGOOD get keyevent thread
    std::thread t1(&UserInputManager::thread_function, this);
    t1.join();
