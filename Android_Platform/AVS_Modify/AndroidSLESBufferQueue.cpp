...
void AndroidSLESBufferQueue::onBufferCompleted() {
    ...
    #if 1 //syoh debug mic buffer output
        int16_t *dump_data = (int16_t *)m_buffers[m_index].data();
    ACSDK_WARN(LX("----------------->>>audio_data").d("reason", "data_audio").d("mic_data ", dump_data[0]).d("mic_data ", dump_data[1]).d("mic_data ", dump_data[2]));
