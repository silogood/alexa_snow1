...
SLDataSink AndroidSLESMicrophone::createSinkConfiguration() {
    ...
    static SLDataFormat_PCM format_pcm = {SL_DATAFORMAT_PCM,            // PCM Format
                                          2,                            // 1 Channel
                                          SL_SAMPLINGRATE_48,           // 16 kHz sampling rate
                                          SL_PCMSAMPLEFORMAT_FIXED_16,  // Fixed-point 16-bit samples
                                          SL_PCMSAMPLEFORMAT_FIXED_16,  // 16 bit container
                                          SL_SPEAKER_FRONT_RIGHT | SL_SPEAKER_FRONT_LEFT,      // channel mask
                                          SL_BYTEORDER_LITTLEENDIAN};   // Data endianness
    return SLDataSink{&loc_bq, &format_pcm};
}
...
bool AndroidSLESMicrophone::configureRecognizeMode() {
  ...
  //  m_recorderObject->getInterface(SL_IID_ANDROIDCONFIGURATION
#if 1 //syoh org
    auto presetValue = SL_ANDROID_RECORDING_PRESET_VOICE_RECOGNITION;
#else
    auto presetValue = SL_ANDROID_RECORDING_PRESET_GENERIC;
#endif
...
/*
        auto presetRetrieved = SL_ANDROID_RECORDING_PRESET_NONE;
        auto presetSize = 1*sizeof(SLuint32);
        result =(*configurationInterface)->GetConfiguration(configurationInterface, SL_ANDROID_KEY_RECORDING_PRESET, &presetSize, (void*)&presetRetrieved);
        if (result != SL_RESULT_SUCCESS) {
...
#else   //syoh test
   if (!m_recorderObject->getInterface(SL_IID_ANDROIDCONFIGURATION, &configurationInterface) || !configurationInterface) {
        ACSDK_WARN(LX("configureRecognizeModeFailed")
                       .d("reason", "configurationInterfaceUnavailable")
                       .d("configuration", configurationInterface));
        return false;
...
SLuint32 presetValue = SL_ANDROID_RECORDING_PRESET_VOICE_RECOGNITION;
   ...
   // if (presetValue != SL_ANDROID_RECORDING_PRESET_NONE) {
        auto result = (*configurationInterface)->SetConfiguration(configurationInterface, SL_ANDROID_KEY_RECORDING_PRESET,
                &presetValue, sizeof(SLuint32));
        if (result != SL_RESULT_SUCCESS) {
            ACSDK_WARN(LX("INNO").d("FAIL", "cannot set preset").d("result", result));
        }
